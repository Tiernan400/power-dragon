...
algorithm.c(c,a.length-1,c.length-1))||(f=!1)},balance:(a,d=10)=>{a&&(algorithm.weights=a);algorithm.closest={balance:Number.POSITIVE_INFINITY,diff:Number.POSITIVE_INFINITY};algorithm.autres=[];algorithm.freeWeights=[];algorithm.fixedLeft=[];algorithm.fixedRight=[];ws=algorithm.weights;for(a=0;a<ws.length;a++)ws[a].active&&("left"==ws[a].side?algorithm.fixedLeft.push({n:ws[a].name,w:parseInt(ws[a].weight,10)}):"right"==ws[a].side?algorithm.fixedRight.push({n:ws[a].name,w:parseInt(ws[a].weight,10)}):
"any"==ws[a].side&&algorithm.freeWeights.push({n:ws[a].name,w:parseInt(ws[a].weight,10)}));wLength=[...algorithm.fixedLeft,...algorithm.fixedRight,...algorithm.freeWeights].length;if(1==wLength%2)return"odd";if(wLength>2*d)return"seats";algorithm.splitArray(algorithm.freeWeights,algorithm.fixedLeft.length,algorithm.fixedRight.length,[...algorithm.fixedLeft,...algorithm.fixedRight,...algorithm.freeWeights]);bestDifference=algorithm.closest.diff;newAutres=[];for(d=0;d<algorithm.autres.length;d++)algorithm.autres[d].diff==
bestDifference&&newAutres.push(algorithm.autres[d]);algorithm.autres=newAutres;return{closest:algorithm.closest,alternatives:algorithm.autres}}};
