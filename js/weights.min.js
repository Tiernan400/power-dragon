window.settings=JSON.parse(file.get(PATH.settings));window.weights=JSON.parse(file.get(PATH.weights));weights=weights.sort(DynamicSort("name"));for(let a=0;a<weights.length;a++)get("list").innerHTML+=`<div>
...
</div>`;0==settings.personalisation.startMaximised&&(get("switch-maximise").click(),menu.getMenuItemById("start-max").checked=!1);0==settings.personalisation.enableHardwareAcceleration&&get("switch-hardware-acceleration").click();window.addEventListener("load",()=>{0==settings.personalisation.server&&get("switch-server").click();get("switch-server").setAttribute("onswitch","updateSettings('personalisation', 'server', !this.vl, true, true);")});
...
<span><input onchange="active(${a}, this)" name="person-check" type="checkbox"${weights[a].active&&" checked"}/></span>
<span><button style="background:red;border-radius:3px;" onclick="deleteWeight(${a});">Delete</button></span>
</div>`;get("balancer").getElementsByClassName("overlay")[0].style.display="flex"}function deleteWeight(a){confirm("Are you sure that you want to delete "+weights[a].name+" from this list?",`(function(res) {
        if (res == 1) {
            weights.splice(${a}, 1);
            updateWeights();
        }
