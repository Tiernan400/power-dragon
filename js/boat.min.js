...
get("balancer").getElementsByClassName("overlay")[0].style.display="flex";get("boat-container").innerHTML='<div class="boat"></div>';if(-1!=settings.boat){get("dynamic-root").innerText=`:root { --part-height: ${100/settings.boats[settings.boat].seats}%; }`;boatPos=[];for(a=0;a<2*parseInt(settings.boats[settings.boat].seats,10);a++)a%2&&(getClass("boat")[0].innerHTML+='<div class="part"></div>'),boatPos[a]="SPACE",get("boat-container").innerHTML+=`<div class="label"><span>${1+a}</span><b></b></div>`;
